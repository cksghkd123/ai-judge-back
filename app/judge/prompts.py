...
# ---------------------------------------------------------------------------
# 출력 내용 가이드: 형식 구조(키·타입)는 JudgmentResult 스키마가 강제하므로,
# 여기서는 judgment_content 의 '내용/서식' 규칙만 안내한다.
# ---------------------------------------------------------------------------
OUTPUT_GUIDANCE = """
판단 요지(judgment_content)는 Markdown으로 보기 좋게 작성하세요. (예: ## 제목, - 목록, 1. 번호)
문단 구분은 빈 줄로 합니다.
청구인·응답인 과실 비율은 두 값의 합이 반드시 100이 되어야 하며,
그 비율을 정한 이유를 본문에서 한 문단 이상으로 설명하세요(숫자만 덧붙이지 마세요).

아래 소제목을 순서대로 포함하세요(에이전트 톤에 맞게 제목 표현은 바꿔도 됨):
1) ## 사실·쟁점 요약 — 제목·논점·양측 주장을 한눈에 정리
2) ## 증거·반박 평가 — [증거 n]을 구체적으로 인용하며, 각 반박(수용/불수용)이 판단에 미친 영향을 짚을 것
3) ## 과실 비율 산정 — 과실 비율 숫자와 본문 서술이 모순되지 않게 근거를 제시
4) ## 결론 및 권고 — 실천 가능한 한두 가지를 제안(비난 일변도로 끝내지 말 것)

증거가 매우 적으면 그 한계를 밝히고, 있는 범위 안에서만 단정하세요.
""".strip()
